...
function MainPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      // [수정] Vultr IP 대신 Netlify Functions 경로 사용 (/api/upload-log)
      const response = await fetch('/api/upload-log', { // <-- 주소 변경!
        method: 'POST',
        body: formData,
        // FormData를 보낼 때는 Content-Type 헤더를 직접 설정하지 않습니다.
        // 브라우저가 boundary를 포함한 올바른 헤더를 자동으로 설정합니다.
      })
